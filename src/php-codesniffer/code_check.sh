...
if [ -n "$RESULT" ]; then
  # 自動修正して確認を促す。
  echo $'\e[1;33mWARNING\e[0mコーディング規約に準拠していないソースコードがあります。\e[0m'
  echo $'\e[1;33m自動修正を行いますが、修正点を確認してからcommitしてください。\e[0m'
  FIX=$(php ./vendor/bin/phpcbf --standard=./phpcs-rule.xml .)
  git diff
  echo $'\e[36mコーディング規約に準拠していないソースコードを修正しました。\e[0m'
  exit 1
fi
