...
RESULT=$(php ./vendor/bin/phpcs --standard=./.git_hooks/coding_check/phpcs-rule.xml .)
if [ -n "$RESULT" ]; then
  ...
  if [[ $RESULT =~ 'FOUND 1 ERROR' ]]; then
    ...
    php ./vendor/bin/phpcbf --standard=./.git_hooks/coding_check/phpcs-rule.xml .
  ...
  if [[ ! $RESULT =~ 'FOUND 1 ERROR' ]] && [[ $RESULT =~ 'FOUND 0 ERRORS' ]]; then
    ...
    php ./vendor/bin/phpcs --standard=./.git_hooks/coding_check/phpcs-rule.xml --report=code .
    exit 1
  fi
