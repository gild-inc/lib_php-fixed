...
RESULT=$(php ./vendor/bin/phpcs --standard=./vendor/gild-inc/git-hooks/laravel/php-codesniffer/rule.xml --colors .)
if [ -n "$RESULT" ]; then
    # コーディングエラーがあれば自動修正して確認を促す。
    echo $'\e[1;35mWARNING\e[0m コーディングエラーがあります。'
    echo $'\e[1;37m自動修正します。\e[0m'
    FIX=$(php ./vendor/bin/phpcbf --standard=./vendor/gild-inc/git-hooks/laravel/php-codesniffer/rule.xml --colors .)
    git diff
